...
|---|---|---|---|---|---|---|---|---|---|
| 1 | Visual battlemap & tokens | 🟡 grid in CombatHUD + terrain overlay | draggable tokens, fog, range preview | pure view-model over engine `BattleMap`; tap → suggested command (no mutation); thin renderer | M | High | ✅ `battlemap-view.test.ts` (view-model + interaction) | renderer wiring + visual | 🟡 deterministic core done; renderer wiring deferred |
