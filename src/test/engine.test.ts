...
describe('engine', () => {
  ...
  it('handles spell slots correctly', () => {
    ...
    // Cast spell level 1
    const out1 = resolveEngineRequest(state, { kind: 'cast_spell', spellName: 'Magic Missile', level: 1 });
    expect(out1.result.ok).toBe(true);
    expect(out1.state.player.spellSlots[1].remaining).toBe(0);

    // Try to cast again with no slots
    const out2 = resolveEngineRequest(out1.state, { kind: 'cast_spell', spellName: 'Magic Missile', level: 1 });
